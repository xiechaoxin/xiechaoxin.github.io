...
![](rocketmq-intro/16ef37fee7e09230.jpg)
...
![](rocketmq-intro/16ef380429cf373e.jpg)
...
![](rocketmq-intro/006APoFYly1fvd9cwjlfrj30as0b03ym.jpg)
...
![](rocketmq-intro/16ef38124f55eaea.jpg)
...
![](rocketmq-intro/16ef382674b66892.jpg)
...
![](rocketmq-intro/16ef382a9756bb1c.jpg)
...
![](rocketmq-intro/16ef382d709abc9d.png)

别急，办法总是有的。

## RocketMQ是什么？

![](rocketmq-intro/16ef383014430799.jpg)
...
![](rocketmq-intro/16ef3834ae653469.jpg)
...
![](rocketmq-intro/16ef3837887d9a54sds.jpg)
...
![](rocketmq-intro/16ef383d3e8c9788.jpg)
...
![](rocketmq-intro/16ef3850c808d707.jpg)

**每个消费组在每个队列上维护一个消费位置** ，为什么呢？

因为我们刚刚画的仅仅是一个消费者组，我们知道在发布订阅模式中一般会涉及到多个消费者组，而每个消费者组在每个队列中的消费位置都是不同的。如果此时有多个消费者组，那么消息被一个消费者组消费完之后是不会删除的(因为其它消费者组也需要呀)，它仅仅是为每个消费者组维护一个 **消费位移(offset)** ，每次消费者组消费完会返回一个成功的响应，然后队列再把维护的消费位移加一，这样就不会出现刚刚消费过的消息再一次被消费了。

![](rocketmq-intro/16ef3857fefaa079.jpg)

可能你还有一个问题，**为什么一个主题中需要维护多个队列** ？

答案是 **提高并发能力** 。的确，每个主题中只存在一个队列也是可行的。你想一下，如果每个主题中只存在一个队列，这个队列中也维护着每个消费者组的消费位置，这样也可以做到 **发布订阅模式** 。如下图。

![](rocketmq-intro/16ef38600cdb6d4b.jpg)
...
- `Broker`： 主要负责消息的存储、投递和查询以及服务高可用保证。说白了就是消息队列服务器嘛，生产者生产消息到 `Broker` ，消费者从 `Broker` 拉取消息并消费。
  ...
  **一个 `Topic` 分布在多个 `Broker`上，一个 `Broker` 可以配置多个 `Topic` ，它们是多对多的关系**。 
  ...
  ![](rocketmq-intro/16ef38687488a5a4.jpg)
...
![](rocketmq-intro/16ef386c6d1e8bdb.jpg)
...
![](rocketmq-intro/16ef386fa3be1e53.jpg)
...
![](rocketmq-intro/16ef3874585e096e.jpg)
...
![](rocketmq-intro/16ef38798d7a987f.png)
...
![](rocketmq-intro/16ef387d939ab66d.jpg)
...
![](rocketmq-intro/16ef387fba311cda.jpg)
...
![](rocketmq-intro/16ef3884c02acc72.png)
...
![](rocketmq-intro/16ef388763c25c62.jpg)
...
![](rocketmq-intro/e314ee45gy1g05zgr67bbj20gp0b3aba.jpg)
